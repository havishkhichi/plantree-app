  ...
  return (
    <div className="flex flex-col gap-4 h-full">
      ...
      <div className="flex gap-4 flex-1 min-h-0" style={{ height: 'calc(100vh - 220px)' }}>
        {/* Map */}
        <div className="relative flex-1 rounded-xl overflow-hidden border border-slate-200 bg-slate-100 shadow-sm">
          <ClimateMap
            hotspots={hotspots}
            layers={layers}
        ...
        <div className="w-72 flex flex-col gap-2 overflow-y-auto shrink-0">
          <div className="flex items-center justify-between px-1">
            ...
            <div className="flex items-center gap-2 bg-white px-2 py-1 rounded border border-slate-200">
              <label htmlFor="dashZoneCount" className="text-[10px] text-slate-500 font-medium">Limit:</label>
              <input 
                id="dashZoneCount"
                type="number" 
                min="3" 
                ...
                className="w-10 bg-transparent text-slate-800 text-xs font-bold text-center border-none focus:outline-none focus:ring-1 focus:ring-emerald-500 rounded p-0"
              />
            </div>
          </div>
          {hotspots.map((h) => (
